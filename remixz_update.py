...
def apply_update(
    ...
    try:
        ...
        if deferred > 0 or pending_dir.exists():
            ...
            # Launcher EXE (3.5.15+): recarga App.py del disco al reiniciar.
            # Preferir EXE/launcher; si no hay, VBS/pythonw + App.py o launcher.py.
            exe = base / "RemixZ_Cleaner_X.exe"
            vbs = base / "ejecutar_Cleaner_X.vbs"
            py = base / "RemixZ_Cleaner_X_App.py"
            launcher_py = base / "launcher.py"
            pending_exe = pending_dir / "RemixZ_Cleaner_X.exe"
            if getattr(_sys, "frozen", False):
                restart_cmd = [_sys.executable]
            elif pending_exe.exists() or exe.exists():
                restart_cmd = [str(exe if exe.exists() else pending_exe)]
            elif vbs.exists() and py.is_file():
                restart_cmd = ["wscript.exe", str(vbs)]
            elif launcher_py.is_file():
                restart_cmd = [_sys.executable, str(launcher_py)]
            elif py.is_file():
