...
def _force_load_disk_module(modname: str, filename: str):
    """
    Carga un .py desde disco (APP_DIR) aunque el EXE tenga el módulo frozen.
    PyInstaller registra módulos en el archive y sys.path NO los reemplaza;
    hay que forzar con importlib.util.
    """
    candidates = [
        APP_DIR / filename,
    ...
    if path is None:
        return None
    # Preferir solo disco APP_DIR / _internal actualizable (no re-cargar frozen idéntico)
    try:
        if modname in sys.modules:
            del sys.modules[modname]
        # submódulos residuales
...
REQUIRED_PACKAGES: list[tuple[str, str, str]] = [
    ...
    ("easygui", "easygui", "Diálogos sencillos (DJ Tools / utilidades)"),
]
...
def ensure_packages(status_cb=None, progress_cb=None) -> dict:
    ...
    # Motor bloqueante; UI casi bloqueante (se reintenta instalar antes de cargar)
    # Solo darkdetect/packaging se toleran si CTk no está; CTk y easygui se intentan siempre.
    optional_ui = {"darkdetect", "packaging"}
    ...
    if not hard_missing and not soft_missing:
        final["ok"] = True
        status(
            "Dependencias listas: mutagen · colorama · psutil · customtkinter · easygui"
        )
    elif not hard_missing:
        final["ok"] = True
        left = ", ".join(m[0] for m in soft_missing)
        status(f"Deps OK (opcionales pendientes: {left})")
    else:
        final["ok"] = False
        left = ", ".join(m[0] for m in hard_missing)
        status(f"Aún faltan dependencias: {left}")
    progress(100)
    return final
...
def _pythonw_for_script() -> str | None:
    """pythonw/python del sistema para correr App.py actualizado (no el EXE frozen)."""
    ...
    if not getattr(sys, "frozen", False):
        ...
        elif exe.name.lower() == "pythonw.exe":
...
def restart_application() -> None:
    """
    Cierra la app y la relanza. Tras un update SIEMPRE se usa esto.

    Arquitectura LAUNCHER (3.5.15+):
      RemixZ_Cleaner_X.exe = launcher.py compilado.
      Solo inyecta path e importa RemixZ_Cleaner_X_App.py DESDE DISCO.
      Por tanto, si estamos frozen, relanzar el MISMO .exe YA carga el App.py
      actualizado por GitHub (no hace falta pythonw salvo sin EXE).

    Orden de preferencia:
      1) EXE launcher (frozen o RemixZ_Cleaner_X.exe en carpeta)
      2) VBS / bat (pythonw + App.py)
      3) pythonw + App.py / launcher.py

    Si hay _pending_update (WinError 32), usa finish_update.cmd.
    """
    cwd = str(APP_DIR)
    cmd: list[str] = []
    try:
        ...
        exe_path = APP_DIR / "RemixZ_Cleaner_X.exe"
        vbs = APP_DIR / "ejecutar_Cleaner_X.vbs"
        bat = APP_DIR / "ejecutar_Cleaner_X.bat"

        # 1) Launcher EXE: recarga App.py del disco automáticamente
        if getattr(sys, "frozen", False):
            cmd = [sys.executable]
        elif exe_path.is_file():
            cmd = [str(exe_path)]
        elif script.is_file() and os.name == "nt" and vbs.exists():
            cmd = ["wscript.exe", str(vbs)]
        elif script.is_file() and os.name == "nt" and bat.exists():
            cmd = ["cmd.exe", "/c", str(bat)]
        elif launcher_py.is_file():
            py = _pythonw_for_script() or sys.executable
            if py == "py":
                cmd = ["py", "-3", str(launcher_py)]
            else:
                cmd = [py, str(launcher_py)]
        elif script.is_file():
            py = _pythonw_for_script()
            if py == "py":
                cmd = ["py", "-3", str(script)]
            elif py:
                cmd = [py, str(script)]
...
def main():
    ...
    def finish_boot(force: bool = False):
        """Cierra splash y muestra bienvenida SOLO tras deps (o force por timeout largo)."""
        if _shown["done"]:
            return
        # No abrir la app si aún se instalan dependencias (evita cargar a medias)
        if not force and not _boot["worker_done"]:
            return
        _shown["done"] = True
        _close_splash()
        try:
    ...
    def boot_worker():
        """Deps PRIMERO, luego full/motor. No cargar app sin dependencias."""
        ...
        try:
            ...
            def _run_deps(label: str, pct_base: int) -> dict:
                try:
                    if splash is not None:
                        app.after(
                            0,
                            lambda: splash.set_status(
                                label, pct_base, step=1
                            ),
                        )
                except Exception:
                    pass
                ui_status(label)
                rep = ensure_packages(status_cb=ui_status, progress_cb=ui_progress)
                _log_boot(
                    f"deps [{label}]: ok={rep.get('ok')} missing={rep.get('missing')}"
                )
                return rep
            ...
            try:
                ...
                if splash is not None:
                    app.after(
                        0,
                        lambda: splash.set_status(
                            "Aplicando update pendiente…", 5, step=0
                        ),
                    )
            except Exception:
                pass
            try:
                ...
                def _pend_progress(pct: int, msg: str = ""):
                    mapped = 5 + int(max(0, min(100, pct)) * 0.08)
                    ui_progress(mapped)
                    if msg:
                        ui_status(msg)
            ...
            # ── 1) DEPENDENCIAS ANTES DE CARGAR (vendor → pip → PyPI) ──
            dep_report = _run_deps("Instalando dependencias faltantes…", 12)
            try:
            ...
            # ── 2) Soft bootstrap → FULL (después de deps básicas) ──
            try:
                ...
                if splash is not None:
                    app.after(
                        0,
                        lambda: splash.set_status(
                            "Descargando paquete completo…", 35, step=0
                        ),
                    )
                ...
                def _full_progress(pct: int, msg: str = ""):
                    mapped = 35 + int(max(0, min(100, pct)) * 0.25)
                    ui_progress(mapped)
                    if msg:
                        ui_status(msg)
            ...
            # ── 3) DEPS de nuevo (por si el FULL trajo requirements nuevos) ──
            dep_report = _run_deps("Verificando dependencias tras update…", 65)
            try:
                if ensure_ctk_loaded(dict(FLUENT)):
                    def _enable_ctk2():
                        try:
                            ...
                            pass
                    app.after(0, _enable_ctk2)
            except Exception:
                pass
            try:
                app.after(
                    0,
                    lambda: setattr(app, "_dep_report", dep_report)
                    or app.refresh_dep_ui(dep_report),
                )
            except Exception:
                app._dep_report = dep_report

            # ── 4) Solo cargar motor si deps de motor están OK ──
            hard_names = {
                m[0]
                for m in (dep_report.get("missing") or [])
                if m[0] not in {"darkdetect", "packaging"}
            }
            if hard_names:
                boot_error = (
                    "Faltan dependencias y no se pudo instalar: "
                    + ", ".join(sorted(hard_names))
                    + ". Revisa Internet o la carpeta vendor/lib."
                )
                _log_boot(boot_error)
                ui_status("Dependencias incompletas")
            else:
                ...
                try:
                    if splash is not None:
                        app.after(
                            0,
                            lambda: splash.set_status(
                                "Cargando motor Cleaner…", 85, step=2
                            ),
                        )
                except Exception:
                    pass
            ...
            _boot["worker_done"] = True
            try:
                app.after(100, finish_boot)
            except Exception:
                pass

    # Failsafe largo: solo si el boot se cuelga de verdad (deps pueden tardar)
    app.after(120000, lambda: finish_boot(force=True))
